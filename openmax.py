...
class Openmax(nn.Module):
    ...
    def load_weight(self, weight_loc):
        ...
        print('loaded weights from {}, epoch {}'.format(weight_loc, checkpoint['epoch']))
    ...
    def set_dataloader(self, base_path, ifft=False):
        ...
        #     drop_last=True,
        # )
        self.dataloader_close_test = torch.utils.data.DataLoader(
            test_set,
            batch_size=1,
            shuffle=True,
            num_workers=NUM_WORKERS,
    ...
    def get_predicted(self, total_scores, print_opt, test_old=False):
        ...
        if test_old == False:
            ...
            predicted[min_score > self.threshold] = 10
        if (print_opt):
...
def parse_args():
    parser = argparse.ArgumentParser(description='openmaxproultra')
    parser.add_argument('--out', type=int, default=0, help='which class to out')
...
def parse_path(args):
    ...
    # if out == 0:
    #     out = 10
    ANNOTATIONS_DIR = 'openset_annotations/annotations{}/'.format(out - 1)
    if out == 0:
        ANNOTATIONS_DIR = 'dataRCS/annotations/'
        weight = 'model_last_.pth'
        ...
    elif out == 1:
        weight = 'RESULTS/1_class_out/model_48.pth'
    elif out == 2:
        weight = 'RESULTS/2_class_out/model_50.pth'
    elif out == 3:
        weight = 'RESULTS/3_class_out/model_37.pth'
    elif out == 4:
        weight = 'RESULTS/4_class_out/model_47.pth'
    elif out == 5:
        weight = 'RESULTS/5_class_out/model_52.pth'
    elif out == 6:
        weight = 'RESULTS/6_class_out/model_52.pth'
    elif out == 7:
        weight = 'RESULTS/7_class_out/model_44.pth'
    elif out == 8:
        weight = 'RESULTS/8_class_out/model_46.pth'
    elif out == 9:
        weight = 'RESULTS/9_class_out/model_55.pth'
    elif out == 10:
        weight = 'RESULTS/10_class_out/model_47.pth'
    else:
        weight = 'RESULTS/{}_class_out/model_best.pth'.format(out)
        ...
    return ANNOTATIONS_DIR, weight


if __name__ == "__main__":
    args = parse_args()
    ANNOTATIONS_DIR, weight = parse_path(args)
    alpha = 5
    num_classes = NUM_CLASSES
    anchors = torch.diag(torch.Tensor([alpha for i in range(num_classes)]))
    classifier = Openmax(weight, base_path=ANNOTATIONS_DIR)
    ...
    (
        ...
        classifier.dataloader_close_train,
        data_idx=1,
    ...
    if test == 'dist':
        ...
        for test_th in th:
            ...
            print(test_th, accuracy_open)
        for test_th in np.linspace(0.34, 1.6, 127):
            classifier.set_threshold(test_th)
            test_predicted = classifier.get_predicted(total_scores, False)
            accuracy_open = acc_predicted(test_predicted, total_gt)
            print('reject2all: ', test_th, accuracy_open)
            test_predicted = classifier.get_predicted(total_scores, False, test_old=True)
            accuracy_open = acc_predicted(test_predicted, total_gt)
            print('reject2old: ', test_th, accuracy_open)
    else:
        # Conf reject test
        classifier.set_threshold(0)
