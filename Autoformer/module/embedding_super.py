...
from Autoformer.utils import to_2tuple
import numpy as np

class PatchembedSuper(nn.Module):
    def __init__(self, img_size=(401,512), patch_size=(20,32), kernel_size=16, in_chans=2, embed_dim=768, scale=False):
    ...
    def set_sample_config(self, sample_embed_dim):
        ...
        # inherit weight and bias from nn.Conv2d
        self.sampled_weight = self.proj.weight[:sample_embed_dim, ...].cuda()
        self.sampled_bias = self.proj.bias[:self.sample_embed_dim, ...].cuda()
        if self.scale:
